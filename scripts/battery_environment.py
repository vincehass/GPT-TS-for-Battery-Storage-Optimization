...
class Battery(gym.Env):
	...
	def __init__(self, env_settings):
		...
		self.idx_ref = 1
		self.bug = False
		self.true_prices = pd.read_csv('../battery-optimisation-with-drl/data/da_prices_train.csv').iloc[:,-1]
		...
		if os.path.isfile(self.torch_file):
			self.model = LSTMCNNModel().cpu()
			#self.model = net
			self.model.load_state_dict(torch.load(self.torch_file, map_location=device))
...
# load test or train price data - should hold in memory (no need to batch)
		if self.train == True:
			...
			self.input_prices = load(test_load);print(self.input_prices)
            
			test_load.close()
        
        #print(self.input_prices)
		...
		with open(f"../battery-optimisation-with-drl/scripts/save_ckpt/train_true_scaler.pkl", "wb") as scaler_store:
