...
class LSTMCNNModel(nn.Module):
	def __init__(self):
		...
		self.cnn = nn.Conv1d(7, 24, kernel_size=24, stride=24)
...
def main():
	...
	train_set_load = open("/Users/nhassen/Documents/ProjectQuant/MyRepos/battery-optimisation-with-drl/data/processed_data/train_data_336hr_in_24hr_out_unshuffled.pkl", "rb") 
	...
	# save trained model
	PATH = '/Users/nhassen/Documents/ProjectQuant/MyRepos/battery-optimisation-with-drl/models/da_price_prediction.pt'
	...
	model.eval()
	with torch.no_grad():
		a = model(torch.unsqueeze(inputs[0].float(),0))
